...
public class DecryptedPagesTest extends SeleniumBaseTest {
    ...
    @Inject
    private TallyingAuthoritiesAggregatePublicKeyPage tallyingAuthoritiesAggregatePublicKeyPage;
    @Inject
    private CandidateOptionSelectedAndEncryptionRandomnessPage candidateOptionSelectedAndEncryptionRandomnessPage;
    @Inject
    private EncryptedBallotPage encryptedBallotPage;
    @Inject
    private BallotSHA256HashPage ballotSHA256HashPage;
    @Inject
    private ThreeWordMemoByBIPMnemonicCodeAlgorithmPage threeWordMemoByBIPMnemonicCodeAlgorithmPage;

    @DataProvider
    public Object[][] getDecryptedBallotSubPages() {
        return new Object[][]{
                {tallyingAuthoritiesAggregatePublicKeyPage},
                {candidateOptionSelectedAndEncryptionRandomnessPage},
                {encryptedBallotPage},
                {ballotSHA256HashPage},
                {threeWordMemoByBIPMnemonicCodeAlgorithmPage}
        };
    }


    /**
     * idea of test is to visit each DecryptedBallotSubPage with help of DataProvider
     */
    @Test(dataProvider = "getDecryptedBallotSubPages")
    public void navigationOnDecryptedBallotSubPagesTest(IPage decryptedBallotSubPage) {
        log.info("current page: " + decryptedBallotSubPage.getClass().getSimpleName());
        decryptedBallotSubPage.defaultNavigateTo();
    }
